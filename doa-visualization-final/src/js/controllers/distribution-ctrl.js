...
angular
    .module('RDash')
    .controller('DistributionCtrl', ['$scope', '$stateParams', '$location', '$state', DistributionCtrl]);

function DistributionCtrl($scope, $stateParams, $location, $state) {
  $('.page-subtitle').text('Home / Distribution');

  $scope.changeRepository = function(selectedRepository) {
    $location.path('/distribution/'+selectedRepository);
    drawGraph(selectedRepository);
	}

  function drawGraph(repo_name) {
    ...
    var file_name = repo_name || "data/repositories/activeadmin.json";
    if (repo_name) {
      if (repo_name == 'Haml') {
        file_name = "data/repositories/haml.json";
      } else if (repo_name == 'ActiveAdmin') {
        file_name = "data/repositories/activeadmin.json";
      }
    }
